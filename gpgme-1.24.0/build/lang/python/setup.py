...
version = version_raw = "1.24.0-unknown"
if '-' in version:
...
if not gpgme_h:
    gpgme_h = os.path.join('/usr/local', 'include', 'gpgme.h')

define_macros = []
if '-L${libdir} -lgpgme':
    libs = '-L${libdir} -lgpgme'.split(' ')
else:
    libs = []
...
if '-I${includedir}':
    for item in '-I${includedir}'.split(' '):
